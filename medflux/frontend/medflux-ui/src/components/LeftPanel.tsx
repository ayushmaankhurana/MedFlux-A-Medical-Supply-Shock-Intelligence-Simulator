      ...
      <div className="p-6 space-y-8">
        ...
        <section>
          ...
          <div className="space-y-5">
            <div>
              <label className="block text-base text-slate-400 mb-2">Budget ($)</label>
              <input
                type="number"
                min="0"
            ...
            <div>
              <label className="block text-base text-slate-400 mb-2">Shelf Life (Days)</label>
              <input
                type="number"
                min="0"
                value={settings.shelf_life_days}
                onChange={(e) => setSettings({ ...settings, shelf_life_days: Number(e.target.value) })}
                className="w-full bg-slate-800 border border-slate-700 rounded-lg px-4 py-3 text-lg text-white focus:border-blue-500 outline-none"
              />
            </div>
          </div>
        ...
        <section>
          ...
          <div className="space-y-5">
            <div>
              <label className="block text-base text-slate-400 mb-2">Shock Type</label>
              <select
                value={settings.shock_type_selected}
                onChange={(e) => setSettings({ ...settings, shock_type_selected: e.target.value as any })}
                className="w-full bg-slate-800 border border-slate-700 rounded-lg px-4 py-3 text-lg text-white focus:border-blue-500 outline-none"
              >
                {SHOCK_TYPES.map((t) => (
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setSettings({ ...settings, shock_magnitude: Number(e.target.value) })}
                className="w-full accent-orange-500 h-2 bg-slate-800 rounded-lg appearance-none cursor-pointer"
              />
            </div>

            <div>
              <label className="block text-base text-slate-400 mb-2">Target Node</label>
              <select
                value={settings.shocked_node_id || ''}
                onChange={(e) => setSettings({ ...settings, shocked_node_id: e.target.value })}
                className="w-full bg-slate-800 border border-slate-700 rounded-lg px-4 py-3 text-lg text-white focus:border-blue-500 outline-none"
              >
        ...
        <section className="bg-slate-800/50 p-5 rounded-xl border border-slate-700">
          <div className="flex justify-between items-center mb-4">
            ...
            <span className="text-blue-400 font-bold text-lg">Day {timelineDay}</span>
          </div>
          <input
            type="range"
            min="0"
            ...
            step="1"
            value={timelineDay}
            onChange={(e) => { if (!isPlaying) setTimelineDay(Number(e.target.value)); }}
            disabled={isPlaying}
          ...
          <section className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              <div className="bg-slate-800 p-4 rounded-xl border border-slate-700 flex flex-col justify-center items-center text-center">
                <span className="text-slate-400 text-base mb-1">Recovery Time</span>
              ...
              <div className={`bg-slate-800 p-4 rounded-xl border border-slate-700 flex flex-col justify-center items-center text-center transition-all duration-500 ${simulationResult.baseline.metrics.peak_shortage > 0 ? 'border-red-500/50 shadow-glow-red' : ''}`}>
                <span className="text-slate-400 text-base mb-1">Peak Shortage</span>
                <span className={`text-3xl font-bold ${simulationResult.baseline.metrics.peak_shortage > 0 ? 'text-red-400 text-glow-critical' : 'text-white'}`}>
            ...
            {simulationResult.optimization_plan?.recommendations && (
              <div className="bg-blue-900/20 border border-blue-800/50 rounded-xl p-5">
                ...
                {simulationResult.executive_summary && (
                  <p className="text-slate-300 text-base italic mb-4 border-l-2 border-blue-500 pl-3">
                    {simulationResult.executive_summary}
                  </p>
                )}

                <ul className="space-y-3">
                  {simulationResult.optimization_plan.recommendations.map((rec, i) => (
                    <li key={i} className="text-slate-300 text-base flex items-start gap-2">
